...
def generate_original_boxes(bbox_reg, scale):
    ...
    bboxes[0, :, :, 0] = (x * 2+1) / scale
    bboxes[0, :, :, 1] = (y * 2+1) / scale
...
def bboxes_iou(bboxes1, bboxes2, method="union"):
    ...
    int_h = np.maximum(int_ymax - int_ymin+1, 0.)
    int_w = np.maximum(int_xmax - int_xmin+1, 0.)
    int_vol = int_h * int_w
    # Union volume.
    vol1 = (bboxes1[2] - bboxes1[0]+1) * (bboxes1[3] - bboxes1[1]+1)
    vol2 = (bboxes2[2] - bboxes2[0]+1) * (bboxes2[3] - bboxes2[1]+1)
